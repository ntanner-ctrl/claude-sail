...
- **`epistemic` field:** Captures epistemic session state so a resumed session can reconnect to the same tracking. `session_id` is the active session ID resolved via `epistemic_get_session_id` (from the per-claude-PID marker under `~/.claude/.current-session/`, or null if none). `preflight_complete` indicates whether preflight assessment was submitted. `last_finding_count` is the number of findings logged so far (helps the resumed session gauge how much was captured).
