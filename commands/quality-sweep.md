...
Log via Empirica `finding_log` if available.
...
| All reviewers timeout | Report sweep incomplete. Log via Empirica if available. |
...
- **Insight capture:** If sweep reveals surprising patterns (recurring issues, unexpected vulnerabilities, systemic quality gaps), log via Empirica `finding_log`
