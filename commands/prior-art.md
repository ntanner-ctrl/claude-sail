...
**Insight capture:** If search reveals a surprisingly popular library or pattern not previously known, log via Empirica `finding_log` (prefix with "[Insight] ").
...
- **Insight capture:** If search reveals surprising findings, log via Empirica `finding_log`
